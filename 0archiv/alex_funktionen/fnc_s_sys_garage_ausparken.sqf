...
if (!(s_mission_params select 5)) exitwith {
  [["<t color='#ff0000' size='2'>Diese Garagen-Funktion steht in der aktuellen Mission nicht zur Verfuegung!", "PLAIN", -1, true, true]] remoteExec ["cutText",_spieler];
};
...
private _button_ok_action = format["[lbData [2014, lbcursel 2014],%1,%2,""%3""] remoteexec ['fnc_s_sys_garage_ausparken_abschluss',2];closedialog 0;",_area select 0,_area select 3,_area_str];
...
["class_lb_auswahl_1_2010",_def_display,_def_toptext_idc,_toptext,_def_lb_ctrl,_lb_eintraege,_lb_eintraege_pic,_lb_eintraege_data,_def_button_ok_idc,_button_ok_action,netid _vec_dummy] remoteexec ["fnc_a_sys_garage_spieler_parkt_aus",remoteExecutedOwner];
