...
// # core ----------------------------------------------------------------------------------------------------------------------------- core
s_vertZ_pos_fob = position eo_obj_spcfg_mission;
...
private _punkte_spieler = s_vertZ_punkte_spieler_start + 0;
private _vec = objnull;
private _aktive_zombies = [];
private _aktive_units = [];
while {s_mission_params select 0} do {
  ...
  _aktive_zombies = _aktive_zombies select {!isnull _x};
  _aktive_zombies = _aktive_zombies select {alive _x};
  _aktive_units = _aktive_units select {!isnull _x};
  _aktive_units = _aktive_units select {alive _x};


  // # spawn-zeit erreicht?
  if (time > _zeitstempel_spawn) then {
    s_vertZ_random_spawn call BIS_fnc_arrayShuffle;
    _spawn_typ = selectrandom s_vertZ_random_spawn;
    _zeitstempel_spawn = time + _spawn_zyklus;
  };



  // # ggf zombie-spawn ausloesen
  if ((_spawn_typ == "zombie") && {(count _erstellte_objekte) < s_vertZ_anz_max_objekte} && {(count _aktive_zombies) < (s_vertZ_anz_max_objekte * (s_vertZ_anz_max_zombies/100))}) then {
    _liste_spawnpos_inf call BIS_fnc_arrayShuffle;
    _unit = [selectrandom _liste_spawnpos_inf,s_vertZ_zombieklassen_feind_allgemein] call fnc_s_sys_spawn_unit;
    _unit move (getPosATL _fahne);
    {_unit reveal _x} forEach (allUnits inareaarray (_loc_params select 1));
    _erstellte_objekte pushback _unit;
    _aktive_zombies pushback _unit;
  };//(_spawn_typ == "inf")



  if ((_spawn_typ in ["rad","kette"]) && {(count _erstellte_objekte) < s_vertZ_anz_max_objekte} && {(count _aktive_units) < (s_vertZ_anz_max_objekte * (s_vertZ_anz_max_units/100))}) then {
    call compile format["_vec_klasse = selectrandom s_vertZ_vecklassen_%1;",_spawn_typ];//"O_MRAP_02_hmg_F";
    ...
    if (_anz > 0) then {
      ...
      // + fahrzeug-gruppe erstellen
      _gruppe = creategroup [east,true];
      {_unit = _gruppe createUnit [selectrandom s_vertZ_unitklassen_feind_allgemein, [0,0,0], [], 0, "NONE"];_unit moveInTurret [_vec, _x]} foreach (allTurrets _vec);
      if ((count(fullCrew [_vec, "gunner", true])) > 0) then {_unit = _gruppe createUnit [selectrandom s_vertZ_unitklassen_feind_allgemein, [0,0,0], [], 0, "NONE"];_unit moveingunner _vec};
      if ((count(fullCrew [_vec, "commander", true])) > 0) then {_unit = _gruppe createUnit [selectrandom s_vertZ_unitklassen_feind_allgemein, [0,0,0], [], 0, "NONE"];_unit moveingunner _vec};
      _unit = _gruppe createUnit [selectrandom s_vertZ_unitklassen_feind_allgemein, [0,0,0], [], 0, "NONE"];
      ...
      _aktive_units append (units _gruppe);
      _erstellte_objekte pushback _vec;
      // # cargo
      if ((count(fullCrew [_vec, "cargo", true])) > 0) then {
        _gruppe = creategroup [east,true];
        {_unit = _gruppe createUnit [selectrandom s_vertZ_unitklassen_feind_allgemein, [0,0,0], [], 0, "NONE"];_unit moveincargo _vec} foreach (fullCrew [_vec, "cargo", true]);
  ...
  // # flaggenstatus
  _units_in_flaggenarea = (allunits select {alive _x}) inareaarray s_vertZ_str_m_fahne_area;
  ...
  if (_units_in_flaggenarea_verhaeltnis != 0) then {
    if (((typeof _fahne) == s_vertZ_zielobjekt_klasse_spieler) && {_units_in_flaggenarea_verhaeltnis < 0}) then {
      _pos = position _fahne; deletevehicle _fahne; _fahne = nil;
      _fahne = s_vertZ_zielobjekt_klasse_feind createVehicle _pos;
      _erstellte_objekte pushback _fahne;
      s_vertZ_str_m_fahne setMarkerColor "ColorRed";
    };//((typeof _zielobjekt) == s_vertZ_zielobjekt_klasse_spieler)
    if (((typeof _fahne) == s_vertZ_zielobjekt_klasse_feind) && {_units_in_flaggenarea_verhaeltnis > 0}) then {
      _pos = position _fahne; deletevehicle _fahne; _fahne = nil;
      _fahne = s_vertZ_zielobjekt_klasse_spieler createVehicle _pos;
      _erstellte_objekte pushback _fahne;
      s_vertZ_str_m_fahne setMarkerColor "ColorBlue";
    };//((typeof _zielobjekt) == s_vertZ_zielobjekt_klasse_spieler)
  ...
  s_vertZ_str_m_fahne setmarkertext (format["Punkte: %1%2",100/(s_vertZ_punkte_sieg/(_punkte_spieler +0.000001)),"%"]);

  // # sieg-punktezahl erreicht
  if (_punkte_spieler > s_vertZ_punkte_sieg) then {s_mission_params set [0,false]; _missionsende_resultat = "punkte erreicht"};
...
s_vertZ_pos_fob = [];// # wichtig um missionsteilnahme zu unterbinden
s_vertZ_moerser_pos = [];
s_vertZ_moersersignal_zaehler = 0;
