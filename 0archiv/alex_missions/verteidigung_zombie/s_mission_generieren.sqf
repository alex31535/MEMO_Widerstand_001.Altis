...
if (s_missionsgenerierungen == s_vertZ_missionsgenerierungen_max) then {
  private _parameter_inidbi = [];
    {_parameter_inidbi pushback [_x select 0,_x select 3]} foreach _parameter_uebergabe;
  [s_vertZ_missionskennung,_parameter_inidbi] call fnc_s_sys_missionsparameter_inidbi;
};
...
10 ["Entfernung Infanterie",[["Sehr weit",650],["Weit",550],["Mittel",450],["Nah",350],["Sehr nah",250]],
11 ["Spawnanteil Zombies",reverse _100er,
*/
{_spieler_param pushback ((_x select 3) select 1)} foreach _parameter_uebergabe;


// # beeinflusste globals
s_vertZ_anz_max_zombies = _spieler_param select 11;
s_vertZ_anz_max_units = 100 - s_vertZ_anz_max_zombies;
...
s_vertZ_punkte_sieg = _spieler_param select 4;
s_vertZ_punkte_spieler_start = (_spieler_param select 4)/100;
s_vertZ_anz_max_objekte = (_spieler_param select 2);
s_vertZ_random_spawn = [];
for "_i" from 1 to (_spieler_param select 7) do {s_vertZ_random_spawn pushback "zombie"};
for "_i" from 1 to (_spieler_param select 8) do {s_vertZ_random_spawn pushback "rad"};
for "_i" from 1 to (_spieler_param select 9) do {s_vertZ_random_spawn pushback "kette"};
...
private _fahne = s_vertZ_zielobjekt_klasse_spieler createVehicle (_pos_dir_fahne select 0);
_erstellte_objekte pushback _fahne;
private _marker = createMarker [s_vertZ_str_m_fahne, (_pos_dir_fahne select 0)];
_marker setMarkerType "mil_dot";
_marker setMarkerColor "ColorBlue";
_erstellte_marker pushback _marker;
_marker = createMarker [s_vertZ_str_m_fahne_area, (_pos_dir_fahne select 0)];
...
{
  if ((((position _x) distance _fahne) < ((_spieler_param select 3) + 50)) && {!((position _x) inarea s_vertZ_str_m_fahne_area)}) then {
...
s_vertZ_liste_spieler_spawn_pos = [selectrandom _haeuser_fob_area1_mit_innenbereich] call fnc_a_sys_positionen_innerhalb_haus;
eo_obj_spcfg_mission = s_vertZ_spcfg_obj_klasse createVehicle [0,0,0];
eo_obj_spcfg_mission setposatl (selectrandom s_vertZ_liste_spieler_spawn_pos);
...
{eo_obj_spcfg_mission addBackpackCargoGlobal _x} foreach s_vertZ_rucksaecke;
{eo_obj_spcfg_mission additemCargoGlobal _x} foreach s_vertZ_items;
{eo_obj_spcfg_mission addMagazineCargoGlobal _x} foreach s_vertZ_mags;
...
private _dist = (_spieler_param select 3) + (_spieler_param select 10) - (_DEF_dist_toleranz/2);
private _pos_rel = [];
...
for "_i" from 0 to 360 step 2 do {
  ...
  if ((!(surfaceiswater _pos_rel)) && {(_pos_rel distance eo_obj_spcfg_mission) > (_spieler_param select 10)}) then {
...
for "_i" from 0 to 360 step 1 do {
  //_pos_rel = _fahne getRelPos [s_vertZ_dist_moerser, _i];
  _strassen = (_fahne getRelPos [s_vertZ_dist_moerser, _i]) nearRoads 50;
  if ((count _strassen) > 0) exitwith {s_vertZ_moerser_pos = position(selectrandom _strassen)};
};
if ((count s_vertZ_moerser_pos) == 0) exitwith {
...
// # dedicated: markieren
if (!isdedicated) then {
  createMarker ["m_vert_moerserposition", s_vertZ_moerser_pos];
...
if (!isdedicated) then {
  private _m = "";
  {
    ...
    _m setmarkertext (format["Z_%1",_foreachindex]);
