...
*/
params ["_spieler"];
if (isnil "s_vert_pos_fob") exitwith {
...
if ((count s_vert_pos_fob) == 0) exitwith {
...
_spieler setposatl (selectrandom s_vert_liste_spieler_spawn_pos);

[_spieler,"Killed"] remoteexec ["removeAllEventHandlers",_spieler];

[_spieler,["Fired",{if ((_this select 5) == "SmokeShellPurple") then {[player,(_this select 6)] remoteexec ["fnc_s_vert_moerserziel",2]}}]] remoteexec ["addEventHandler",_spieler];
