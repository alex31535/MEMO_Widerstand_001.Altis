  ...
  if ((_spawn_typ != "") && {(count _erstellte_objekte) < ((_gen_params select 2))}) then {
    ...
    if (_spawn_typ in ["rad","kette"]) then {
      ...
      if (_anz > 0) then {
        ...
        {_unit = _gruppe createUnit [s_feind_klasse, [0,0,0], [], 0, "NONE"];_unit moveInTurret [_vec, _x]} foreach (allTurrets _vec);
        if ((count(fullCrew [_vec, "gunner", true])) > 0) then {_unit = _gruppe createUnit [s_feind_klasse, [0,0,0], [], 0, "NONE"];_unit moveingunner _vec};
        if ((count(fullCrew [_vec, "commander", true])) > 0) then {_unit = _gruppe createUnit [s_feind_klasse, [0,0,0], [], 0, "NONE"];_unit moveingunner _vec};
        ...
        if ((count(fullCrew [_vec, "cargo", true])) > 0) then {
          _gruppe = creategroup [east,true];
          {_unit = _gruppe createUnit [s_feind_klasse, [0,0,0], [], 0, "NONE"];_unit moveincargo _vec} foreach (fullCrew [_vec, "cargo", true]);
          _gruppe addWaypoint [position _fahne, 5, 0];
          [_gruppe, 0] setWaypointBehaviour "AWARE";
          [_gruppe, 0] setWaypointspeed "FULL";
          [_gruppe, 0] setWaypointType "MOVE";
