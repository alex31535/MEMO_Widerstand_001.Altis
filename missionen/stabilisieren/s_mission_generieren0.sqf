...
for "_i" from 0 to 360 step 2 do {
  ...
  if ((!(surfaceiswater _pos_rel)) && {(_pos_rel distance _fahne) > (_spieler_param select 12)}) then {
...
for "_i" from 1 to (_spieler_param select 7) do {_vert_random_spawn pushback "inf"};
for "_i" from 1 to (_spieler_param select 8) do {_vert_random_spawn pushback "rad"};
for "_i" from 1 to (_spieler_param select 9) do {_vert_random_spawn pushback "kette"};
//for "_i" from 1 to (_spieler_param select 10) do {_vert_random_spawn pushback "flug"};
//for "_i" from 1 to (_spieler_param select 11) do {_vert_random_spawn pushback "heli"};
...
private _haeuser_area1 = [(_loc_params select 1) select 0, (_loc_params select 1) select 1, _DEF_min_anz_geb_positionen_fob] call fnc_s_sys_gebaeudeauswahl;
...
private _positionen_innerhalb_haus = [];
{
  _positionen_innerhalb_haus = [_x] call fnc_s_sys_positionen_innerhalb_haus;
...
s_vert_liste_spieler_spawn_pos = [selectrandom _haeuser_fob_area1_mit_innenbereich] call fnc_s_sys_positionen_innerhalb_haus;
...
for "_i" from 0 to 360 step 2 do {
  ...
  if ((!(surfaceiswater _pos_rel)) && {(_pos_rel distance eo_obj_spcfg_mission) > (_spieler_param select 12)}) then {
    _wasser_gefunden = [_pos_rel,position _fahne] call fnc_s_sys_wasser_zwischen_a_und_b;
    if (!_wasser_gefunden) then {
      _pos_im_wald = [_pos_rel] call fnc_s_sys_pos_im_wald;
      if (_pos_im_wald) then {
        _liste_spawnpos_inf pushback _pos_rel;
      } else {
        _gebaeude = [_pos_rel,50,_DEF_min_anz_geb_positionen_inf_spawn] call fnc_s_sys_gebaeudeauswahl;
        if ((count _gebaeude) > 0) then {
          _positionen_innerhalb_haus = [selectrandom _gebaeude] call fnc_s_sys_positionen_innerhalb_haus;
          if ((count _positionen_innerhalb_haus) > 0) then {
            _pos = selectrandom _positionen_innerhalb_haus;
            {if (((count _pos) > 0) && {(_pos distance _x) < 50}) then {_pos = []}} foreach _liste_spawnpos_inf;
            if ((count _pos) > 0) then {_liste_spawnpos_inf pushback _pos};
